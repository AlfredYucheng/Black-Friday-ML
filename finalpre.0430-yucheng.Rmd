...
#sumpur in groups
cuts.pur <- seq(0,10600000,400000)
...
##random forest
```{r 1. random forest}
err <- function(model,train,test,yval){
   test.yhat <- predict(object=model,newdata=test)
   test.y <- with(test,get(yval))
   test.err <- mean((test.yhat-test.y)^2)
   test.err
 }

m =ncol(train_2000)
for (i in 1:(m-1)){
  test_model <- randomForest(sumpur~.,data=train_2000,mtry=i,importance=TRUE, proximity=TRUE)
  mse <- mean(test_model$mse)
  print(mse)
}

forest_model <- randomForest(sumpur~.,data=train_2000,mtry=11,ntree=500)


sqrt(err(forest_model,train_2000,test,'sumpur'))
```
...
multinom.logit <- multinom(factor(pur.group) ~., data=train[,-c(3,7)], maxit=500)
#0.212831
multinom.logit <- multinom(factor(pur.group) ~ ., data=train[,-7], maxit=500) 
#0.2111337

predict.ml <- predict(multinom.logit, test[,-7])

CM <- auc(predict.ml, test$pur.group) 
...
test$pur.group <- factor(train$pur.group)
